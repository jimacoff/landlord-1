...
StripeEvent.configure do |events|
  ...
  events.subscribe 'customer.updated' do |event|
    begin
      ...
      SupportMailer.notification('Stripe event error', "#{ex.message}").deliver_later
    end
  end
  ...
  events.subscribe 'customer.subscription.trial_will_end' do |event|
    ...
    account = Landlord::Account.find_by(stripe_id: subscription.customer) if subscription
    Landlord::AccountMailer.trial_will_end(account).deliver_later if account
  end
  ...
  events.subscribe 'charge.succeeded' do |event|
    ...
    receipt = Landlord::Receipt.save_from_stripe(charge) if charge
    Landlord::ReceiptMailer.succeeded(receipt).deliver_later if receipt
  end
  ...
  events.subscribe 'charge.failed' do |event|
    charge = event.data.object
    receipt = Landlord::Receipt.save_from_stripe(charge) if charge
    Landlord::ReceiptMailer.failed(receipt).deliver_later if receipt
  end
  ...
  events.subscribe 'charge.refunded' do |event|
    charge = event.data.object
    receipt = Landlord::Receipt.save_from_stripe(charge) if charge
    Landlord::ReceiptMailer.refunded(receipt).deliver_later if receipt
  end
